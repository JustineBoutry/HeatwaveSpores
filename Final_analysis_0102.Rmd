...
```{r, echo=FALSE}
# Extraire les noms de groupes du modèle (sauf l'intercept)
cox_df <- broom::tidy(fit_cox_2, exponentiate = TRUE, conf.int = TRUE)
cox_df$term <- gsub("^group", "", cox_df$term)

# Ajoute la colonne de significativité à plusieurs niveaux

# Découpage correct des p-values et labels cohérents
cox_df$signif <- cut(cox_df$p.value,
                     breaks = c(-Inf, 0.051, 0.08, Inf),
                     labels = c("p < 0.051", "p < 0.08", "ns"))

# Compte les effectifs par groupe
