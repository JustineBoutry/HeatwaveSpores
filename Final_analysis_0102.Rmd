...
dataC20$Isolate = factor(dataC20$Isolate, levels= c("unexposed","C1","C14", "C18","C24"), ordered = F)

table(dataC20$Isolate)
...
summary(fit_cox_1)# not the best model, just to see
...
fit_weibull1 = survreg(surv_obj ~ 1, data = dataC20, dist = "weibull")
fit_weibull2 = survreg(surv_obj ~ Isolate, data = dataC20, dist = "weibull")

AICc(fit_weibull1,fit_weibull2) # null model is better

test_weibull2 = residuals(fit_weibull2, type = "matrix")
qqnorm(test_weibull2)
qqline(test_weibull2, col = "red")

fit_exp = survreg(surv_obj ~ Isolate, data = dataC20, dist = "exponential")
fit_loglog = survreg(surv_obj ~ Isolate, data = dataC20, dist = "loglogistic")
fit_lognorm = survreg(surv_obj ~ Isolate, data = dataC20, dist = "lognormal")
fit_gauss = survreg(surv_obj ~ Isolate, data = dataC20, dist = "gaussian")

AICc(fit_weibull2, fit_exp, fit_loglog, fit_lognorm, fit_gauss)
qqnorm(residuals(fit_lognorm))
qqline(residuals(fit_lognorm), col = "red")

# Diagnostic
fit_km = survfit(surv_obj ~ Isolate, data = dataC)
plot(fit_km, col = 1:13, xlab = "Age", ylab = "Survie")
legend("topright", levels(dataC20$Isolate), col = 1:13, lty = 1)
plot(fit_km, fun = "cloglog", col = 1:13)

library(survminer)

# Test global d'abord
pw_test = pairwise_survdiff(Surv(DeathAge, statusDeath) ~ group, 
                             data = dataC,
                             p.adjust.method = "BH")
pw_test
print(pw_test$p.value["CTRLCTRL",])

```

\
We can see that there is no significant difference between the model comparing the survival of each spore lineage (including the group without spores) and the null model. So it seem than at 20°C, in stable conditions, the exposure to dead Daphnias or to spores from different genotype influence similarly the survival of non infected individuals, however we can not than the effective in those categories are sometimes desequilibrated. I doubt of the relevancy of this analysis.

#### Comparision between groups, without factorial modeling
```{r, echo=F}

dataC$group = factor(dataC$group, levels= c("unexposed20","C120","C130","C140","C1420","C1430","C1440","C1820","C1830","C1840",
"C2420","C2430","C2440"), ordered = F)
levels(dataC$group)

surv_obj = Surv(time = dataC$DeathAge, event = dataC$statusDeath)  

fit_weibull1 = survreg(surv_obj ~ 1, data = dataC, dist = "weibull")
fit_weibull2 = survreg(surv_obj ~ group, data = dataC, dist = "weibull")
