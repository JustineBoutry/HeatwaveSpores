---
title: "Ecology - Appendix S2 : Phenotypic plasticity of 'off-host' parasite stages in response to heatwaves in the Daphnia-Pasteuria system"
...
```
\
This supplementary R Markdown document provides the full analytical workflow and visual outputs referenced in the main manuscript "Phenotypic plasticity of 'off-host' parasite stages in response to heatwaves in the Daphnia-Pasteuria system". All analyses were performed in R 4.4.2 (2024-10-31). The dataset and scripts are provided to ensure full reproducibility.
...
n.b : For both Temperature and Isolate, the 'CTRL' category represents unexposed control samples that were not subjected to the experimental conditions and served as husbandry controls.
...
setwd("/Users/justineboutry/Desktop/Applications/CNRS-permanent-position/HeatwaveSpores")
...
data$Isolate = factor(data$Isolate, levels = c("CTRL","C14", "C1", "C18", "C24"), ordered = FALSE)
...
data         = droplevels(subset(data, Temperature != 'CTRL')) # removing unexposed daphnias for the main analysis
...
Control      = droplevels(subset(dataC, Temperature == 'CTRL'))

# correction due to CTRL removal
...
surv_obj_ctrl <- Surv(time = Control$DeathAge, event = Control$statusDeath)
km_fit_ctrl   <- survfit(surv_obj_ctrl ~ Temperature, data = Control)  # ~ 1 car une seule condition
km_df_ctrl    <- surv_summary(km_fit_ctrl, data = Control)
km_df_ctrl$Temperature <- "Unexposed control"

km_df_combined <- rbind(
  km_df_Ninf2[ , intersect(names(km_df_Ninf2), names(km_df_ctrl))],
  km_df_ctrl[  , intersect(names(km_df_Ninf2), names(km_df_ctrl))]
)

ggplot(km_df_combined, aes(x = time, y = surv, color = Temperature)) +
  geom_step(linetype=1, size=1) +
  ...
  ylim(0,1)+
  #facet_wrap( ~ Isolate) +
  scale_color_manual(values=c("chartreuse4", "darkorange", "darkred", "grey40"))+
  scale_fill_manual(values=c("chartreuse4", "darkorange", "darkred","grey40"))+
  theme_minimal() 
```
...
```{r, echo=F}
table(dataC$Isolate, dataC$Temperature)
surv_obj = Surv(time = dataC$DeathAge, event = dataC$statusDeath)
fit_km = survfit(surv_obj ~ Temperature, data = dataC)
plot(fit_km, fun = "cloglog",
     lwd = 2,
      col = c("grey40", "chartreuse4", "darkorange", "darkred"),
     xlab = "log(temps)", 
     ylab = "log(-log(S(t)))",
     main = "Log-log plot - Proportional Hazards check")
```

We start first by running our initial analysis including the husbandry control along with the other genotype.
...
```{r, echo=T}
dataC$Temperature = factor(dataC$Temperature, levels = c("CTRL", 20, 30, 40), ordered = F)
surv_obj = Surv(time = dataC$DeathAge, event = dataC$statusDeath)

fit_cox_1 = coxph(surv_obj ~ 1, data = dataC)
fit_cox_r = coxme(surv_obj ~ 1 + (1|Batch), data = dataC)

AICc(fit_cox_1, fit_cox_r) # Random effect is not significantly improving our results
...
\
We can see that the main effect explaining survival variation is still temperature, with a significant decrease in mortality risk at higher temperatures (p = 0.002). The hazard ratio indicates a 39.4% reduction in mortality risk per temperature unit increase (HR = 0.606). This suggests that even when including the husbandry control, temperature remains a key factor influencing survival outcomes in exposed uninfected Daphnia.

#### Comparision between the husbandry control and the other groups
```{r, echo=T}
dataC$group <- factor(
  paste0(dataC$Isolate, dataC$Temperature),
  levels = c("CTRLCTRL","C120","C130","C140","C1420","C1430","C1440","C1820","C1830","C1840","C2420","C2430","C2440"),
  ordered = FALSE
)
table(dataC$group, dataC$DeathAge)

surv_obj = Surv(time = dataC$DeathAge, event = dataC$statusDeath)

fit_cox_1 = coxph(surv_obj ~ 1, data = dataC)
fit_cox_2 = coxph(surv_obj ~ group, data = dataC)
...
##### Visualisation of comparisions between the husbandry control and the other groups

```{r, echo=FALSE}

cox_df <- broom::tidy(fit_cox_2, exponentiate = TRUE, conf.int = TRUE)
cox_df$term <- gsub("^group", "", cox_df$term)

cox_df$signif <- cut(cox_df$p.value,
                     breaks = c(-Inf, 0.051, 0.08, Inf),
                     labels = c("p < 0.051", "p < 0.08", "ns"))


group_counts <- as.data.frame(table(dataC$group))
colnames(group_counts) <- c("term", "n")

cox_df <- merge(cox_df, group_counts, by = "term", all.x = TRUE)
cox_df$label <- paste0(cox_df$term, " (n=", cox_df$n, ")")

ggplot(cox_df, aes(y = label, x = estimate, xmin = conf.low, xmax = conf.high, color = signif)) +
  geom_point(size = 6) +
  geom_errorbarh(height = 0.2) +
  geom_vline(xintercept = 1, linetype = "dashed") +
  scale_x_log10() +
  scale_color_manual(values = c(
    "p < 0.051" = "#a50026",   # dark red
    "p < 0.08" = "#e66101",    # dark orange
    "ns" = "#4575b4"           # blue
  )) +
  labs(
    title = "Comparision of mortality risk for each group vs husbandry control",
    subtitle = "Reference husbandry control group contains 27 individuals",
...
```{r, echo=TRUE, warning=FALSE, message=F}
options(na.action = "na.fail")
FC1 = glmmTMB(data=dataC, Never_repro~ Isolate + Temperature + DeathAge, family = "binomial", REML=T)
FC1R = glmmTMB(data=dataC, Never_repro ~ Isolate + Temperature+ DeathAge + (1|Batch), family = "binomial", REML=T)

AICc(FC1, FC1R)

FC1 = glmmTMB(data=dataC, Never_repro ~ Isolate * Temperature + DeathAge + (1|Batch), family = "binomial")
model_selection = dredge(FC1)
...
best_model = get.models(model_selection, 3)[[1]]
summary(best_model)
tab_model(best_model)
...
p1 = ggplot(data_cat_summary_Ninf, aes(x = DeathAgeGroup, y = Proportion, fill = Temperature)) +
  ...
  #scale_fill_manual(values = lineage_colors, name = "Parasite Strain") +
  theme_minimal()+
  scale_fill_manual(values=c( "grey40", "chartreuse4", "darkorange", "darkred"))
p1
```
...
FC1 = glmmTMB(data = repro_inf, Nb_offspring ~ Isolate + Temperature + DeathAge, family = truncated_nbinom2, REML = T)

simulateResiduals(FC1, plot=T)

FC1R = glmmTMB(data=repro_inf, Nb_offspring ~ Isolate + Temperature + DeathAge + (1|Batch), family = truncated_nbinom2, REML=T)
AICc(FC1, FC1R)# no batch effect

FC1 = glmmTMB(Nb_offspring ~ Isolate * Temperature + DeathAge  , 
                data = repro_inf, family = truncated_nbinom2, na.action = na.fail)
model_selection = dredge(FC1)
...
#FC1 = glmmTMB(data=repro_nf, Nb_offspring ~ Isolate * Temperature + DeathAge+ (1|Batch), family = truncated_nbinom1, REML=T)
...
```{r, echo=TRUE, warning=FALSE, message=F}
dataC$Isolate = factor(dataC$Isolate, levels=c("CTRL", "C1", "C14", "C18", "C24"), ordered=FALSE)
repro_nfC = subset(dataC, dataC$Clutch>0 & InfectedStatus!="inf")
repro_nfC$Temperature = factor(repro_nfC$Temperature, labels = c("CTRL", "20", "30","40"), ordered = F)
...
selected_models = get.models(model_selection, 1)
...
b = ggplot() +
  geom_jitter(data = data_c2, aes(x = Isolate, y = caulif_n, color = Isolate), 
              width = 0.3, height = 0.1, alpha = 0.5, size=3) +  
  geom_point(data = pred_data, aes(x = Isolate, y = Probability), size = 1, group=1) +  
  geom_errorbar(data = pred_data, aes(x = Isolate, ymin = Lower_CI, ymax = Upper_CI), width=0.2) + 
  facet_grid(cols = vars(Temperature)) +  
  labs(
    x = "Temperature",
...
best_mod = polr(stade_simplifie ~ Isolate + Temperature + DeathAge, 
                data = infected, Hess = TRUE)
tab_model(best_mod)
brant(best_mod) # Not much better, but same concluisons, so I kept the other one.
tab_model(modele14)
```
